...
pub fn break_block(game_data: &mut common::GameData, world_data: &Arc<std::sync::Mutex<world::WorldData>>) -> (Vec<common::Vertex>, i32, Vec<common::Vertex>) {
    ...
    for i in 0..16 {
        ...
        let block_type = chunk::get_block_global(game_data, &world_data_read, 
        ...
        if block_type != 0 && block_type != -1 {
            ...
            if let Some(chunk) = world_data_read.chunks.get(&(chunk_position_x, chunk_position_y, chunk_position_z)) {
                ...
                let light_data = world_data_read.chunks_lighting[&(chunk_position_x, chunk_position_y, chunk_position_z)].clone();
                ...
                let (chunk_vertices, chunk_normals, chunk_colors, chunk_uvs, 
                    ...
                    ) = chunk::render_chunk(&chunk_data, &light_data, &game_data, &world_data_read, 
...

                {
                    ...
                    world_data_reading.set_chunk(chunk_position_x, chunk_position_y, chunk_position_z, chunk_data, light_data);
...
pub fn place_block(game_data: &mut common::GameData, world_data: &Arc<std::sync::Mutex<world::WorldData>>, slot_selected: i8, inventory: containers::Inventory) -> (Vec<common::Vertex>, i32, Vec<common::Vertex>) {
    let world_data_read = world_data.lock().unwrap().clone();
